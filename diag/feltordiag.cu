...
int main( int argc, char* argv[])
{
    ...
    dg::CylindricalGrid3d g3d_out( Rmin,Rmax, Zmin,Zmax, 0, 2.*M_PI, p.n_out, p.Nx_out, p.Ny_out, p.Nz_out, p.bcxN, p.bcyN, dg::PER);
    dg::Grid2d  g2d_out( Rmin,Rmax, Zmin,Zmax, p.n_out, p.Nx_out, p.Ny_out,  p.bcxN, p.bcyN);
    //1d grid
...
//     double psipmax = 0.0;
    ...
    std::string names2d[13] = {"Ne_avg", "Ni_avg", "Ue_avg", "Ui_avg",
    "phi_avg","dNe_mp", "dNi_mp", "dUe_mp", "dUi_mp",
    "dphi_mp","vor_avg","Depsi_avg","Lperpinv_avg"};
    int dataIDs2d[13];
...
//     size_t start3dp[4] = {0, 0, 0, 0};
    ...
    std::string names1d[10] = {"Ne_fsa", "Ni_fsa", "Ue_Fsa", "Ui_fsa",
    "phi_fsa","q","vor_fsa","Depsi_fsa","Lperpinv_fsa","psip1d"};
    int dataIDs1d[10];
