...
int main()
{
    ...
    //HVec grid = dg::positions< double, n>( 0, lx, N);
    HArrVec h_v = dg::expand< double(&) (double), n>( function, 0, lx, N);
    //h_v.data() = dg::evaluate( function, grid);
    HArrVec h_x = dg::evaluate< double(&) (double), n>( function, 0, lx, N);
    ...
    double normX = dg::blas2::dot( h_x.data(), dg::W1D<double, n>(h), h_x.data());
