...
struct Hector
{
    ...
    Hector( Psi psi, PsiX psiX, PsiY psiY, LaplacePsi laplacePsi, double psi0, double psi1, double X0, double Y0, unsigned n, unsigned Nx, unsigned Ny, double eps) : 
        g2d_(psi, psiX, psiY, psi0, psi1, X0, Y0, n, Nx, Ny)
    {
        ...
        while( (eps < eps_old||eps > 1e-7) && eps > 1e-10)
        {
            eps = eps_old;
            Nx*=2, Ny*=2;
            orthogonal::RingGrid2d<container> g2d(gp, psi_0, psi_1, n, Nx, Ny,dg::DIR);
