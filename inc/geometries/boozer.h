...
//         return construct_f( psi, R_0, Z_0);
            ...
            double& R_0, double& Z_0, double& f, double& fp ) 
    {
        ...
        solovev::InvB invB(gp_);

        double psipR_ = psipR( begin[0], begin[1]), psipZ_ = psipZ( begin[0], begin[1]);
        double psip2 = psipR_*psipR_+psipZ_*psipZ_;
        double B=1./invB(begin[0], begin[1]);
        double fnorm = f_psi * B*B;
        //initial conditions:
        double Rfac =  begin[0]/gp_.R_0;

        begin[2] =  Rfac*fnorm*( psipZ_/psip2);        //y_R(R_0,Z_0)
        begin[3] = -Rfac*fnorm*( psipR_/psip2);       //y_Z(R_0,Z_0)

        R_0 = begin[0], Z_0 = begin[1];
        //std::cout <<f_psi<<" "<<" "<< begin[0] << " "<<begin[1]<<"\t";
        solovev::boozer::FieldRZYRYZY fieldRZY(gp_);
    ...
    const solovev::boozer::FieldRZYT fieldRZYT_;
...
struct FieldFinv
{
    ...
    double deltapsi;
    Fpsi fpsi_;
    solovev::boozer::FieldRZYT fieldRZYT_;
...
/**
 * @brief A three-dimensional grid based on "almost-boozer" coordinates by Ribeiro and Scott 2010
 *
...
struct RingGrid3d : public dg::Grid3d<double>
{
    ...
    { 
        ...
        boozer::detail::Fpsi fpsi( gp, psi_0,deltapsi);
    ...
    perpendicular_grid perp_grid() const { return boozer::RingGrid2d<container>(*this);}
...
/**
 * @brief A two-dimensional grid based on "almost-boozer" coordinates by Ribeiro and Scott 2010
 */
template< class container>
struct RingGrid2d : public dg::Grid2d<double>
{
        ...
        dg::Grid2d<double>( 0, 1., 0., 2*M_PI, n,Nx,Ny, bcx, dg::PER)
    {
        ...
        if( x_1 > 0)
            ...
            init_X_boundaries( x_1, 0.);
        boozer::RingGrid3d<container> g( gp, psi_0, psi_1, n,Nx,Ny,1,bcx);
        f_x_ = g.f_x();
...
namespace dg{
/**
 * @brief This function pulls back a function defined in cartesian coordinates R,Z to the boozer coordinates x,y,\phi
 *
...
thrust::host_vector<double> pullback( BinaryOp f, const boozer::RingGrid2d<container>& g)
{
...
///@cond
template<class container>
thrust::host_vector<double> pullback( double(f)(double,double), const boozer::RingGrid2d<container>& g)
{
    return pullback<double(double,double),container>( f, g);
}
///@endcond
/**
 * @brief This function pulls back a function defined in cylindrical coordinates R,Z,\phi to the boozer coordinates x,y,\phi
 *
...
thrust::host_vector<double> pullback( TernaryOp f, const boozer::RingGrid3d<container>& g)
{
    thrust::host_vector<double> vec( g.size());
...
///@cond
template<class container>
thrust::host_vector<double> pullback( double(f)(double,double,double), const boozer::RingGrid3d<container>& g)
{
