...
struct Fpsi
{
    ...
    double construct_f( double psi, double& R_0, double& Z_0) 
    {
        ...
        double f_psi = 2.*M_PI/end_old[2]; //=1/q
        return f_psi;
    }
    double operator()( double psi)
    {
        double R_0, Z_0; 
        return construct_f( psi, R_0, Z_0)*deltapsi;//=deltapsi/q
    ...
    double find_x1( double psi_1 ) 
    {
        ...
        while(eps < eps_old && P < 20 && eps > 1e-15)
        {
            ...
            {
                f_vec[i] =1./this->operator()( psi_vec[i]); // = q/deltapsi
            }
            ...
            double& R_0, double& Z_0, double& f, double& fp ) 
    {
        ...
        while( eps < eps_old)
        {
            ...
            xr[0] = -psipR(r[0],z[0])/f_psi/deltapsi, xz[0] = -psipZ(r[0],z[0])/f_psi/deltapsi; //dx = -q dpsi
            ...
            for( unsigned i=1; i<n*N; i++)
            {
                ...
                xr[i] = -psipR(r[i],z[i])/f_psi/deltapsi, xz[i] = -psipZ(r[i],z[i])/f_psi/deltapsi;//dx = -q dpsi
            }
...
//         fpsiM[0] = - end[2]/2./M_PI;
     fpsiM[0] = - 1./(end[2]/2./M_PI)/deltapsi; // q/ deltapsi
...
/**
 * @brief A three-dimensional grid based on "almost-flux" coordinates by Ribeiro and Scott 2010
 *
...
//         double deltapsi = ( psi_0-psi_1);
        double deltapsi = 1.;
        flux2::detail::Fpsi fpsi( gp, psi_0, deltapsi);
...
/**
 * @brief A two-dimensional grid based on "almost-flux" coordinates by Ribeiro and Scott 2010
 */
template< class container>
...
///@endcond
/**
 * @brief This function pulls back a function defined in cylindrical coordinates R,Z,\phi to the flux coordinates x,y,\phi
 *
