...
int main()
{
    ...
    dg::Elliptic3d<dg::CylindricalGrid3d, dg::DMatrix, dg::DVec> elliptic(g3d, dg::normed, dg::centered);
    ...
    t.tic();
    std::cout << "# Number of pcg iterations "<< invert( test, x, b)<<std::endl;
    t.toc();
    ...
    t.tic();
    std::cout << "# Number of pcg iterations "<< invert( ellipticP, x, b)<<std::endl;
    t.toc();
    std::cout << "# ... on the device took "<< t.diff()<<"s\n";
