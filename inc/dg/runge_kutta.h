...
struct RK_opt<1, container>
{
    ...
    template < class RHS>
    void step( RHS& f, real_type t0, const container& u0, real_type& t1, container& u1, real_type dt)
    {
...
struct RK
{
    ...
    ///@copydoc RK_opt::step(RHS&,real_type,const container&,real_type&,container&,real_type)
    template<class RHS>
    void step( RHS& rhs, real_type t0, const container& u0, real_type& t1, container& u1, real_type dt);
  private:
...
template< size_t s, class container>
template< class RHS>
void RK<s, container>::step( RHS& f, real_type t0, const container& u0, real_type& t1, container& u1, real_type dt)
{
...
void stepperRK(RHS& rhs, get_value_type<container>  t_begin, const container& begin, get_value_type<container> t_end, container& end, unsigned N )
{
    using real_type = get_value_type<container>;
 ...
 * The other function is the \c real_type \c error( const container& end0, const container& end1); which computes the error norm in which the integrator should converge.
 * @copydoc hide_container
 * @param rhs The right-hand-side
 * @param t_begin initial time
 * @param begin initial condition
 ...
 */
template<unsigned s, class RHS, class container>
int integrateRK(RHS& rhs, get_value_type<container> t_begin, const container& begin, get_value_type<container> t_end, container& end, get_value_type<container> eps_abs, unsigned NT_init = 2 )
{
    using real_type = get_value_type<container>;
    RK<s, container > rk( begin);
