...
* 
*       Is unstable for matrix size of roughly > 150. Fails for certain tridiagonal matrix forms
* @ingroup invert
*/
...
class HMGTI
{
  public:
     ...
     */
    HMGTI(const ContainerType& copyable) 
    {
...
class InvTridiag_FD
{
  public:
     ...
     * @param copyable vector
     */
    InvTridiag_FD(const ContainerType& copyable) 
    {
        m_size = copyable.size();
...
//         m_theta[0] = 0.0; 
        ...
        helper = a[m_size-1] + c[m_size-2]*m_phi[m_size-2];
        if (helper==0) std::cout << "Failure: No inverse exists\n" << std::endl;
