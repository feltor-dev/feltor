...
double lhs( double x,double){ return sin(x);}
double rhs( double x,double){
...
double dxrhs( double x,double){ return (1.+x)*sin(x)-2*alpha*sin(x)+alpha*alpha*(2*cos(x)/(1.+x)/(1.+x)-2*sin(x)/(1.+x)/(1.+x)/(1.+x)+sin(x)/(1.+x));} // chi=x
