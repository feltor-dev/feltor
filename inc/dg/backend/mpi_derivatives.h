...
{
    MPI_Matrix dx = detail::dx( g.local(), bcx, g.bcy(), no, dir, g.communicator() );
    if( no == not_normed)
        dx.precond() = dg::create::weights(g);
    return dx;
}

MPI_Matrix dx( const MPI_Grid2d& g, norm no = normed, direction dir = symmetric)
{
    MPI_Matrix dx = detail::dx( g.local(), g.bcx(), g.bcy(), no, dir, g.communicator() );
    if( no == not_normed)
        dx.precond() = dg::create::weights(g);
    return dx;
}
