...
int main( int argc, char * argv[])
{
    ...
    assert( size==0 or dg::is_communicating( messages, MPI_COMM_WORLD));
    ...
    // 5. Send each following rank a vector of std::array
    {
    ...
    auto recv = dg::mpi_permute( messages, MPI_COMM_WORLD);
    assert( recv[(rank+size-1)%size][0][0] == (rank+size-1)%  size);
    auto messages_num = dg::mpi_permute( recv, MPI_COMM_WORLD);
    assert( messages == messages_num);
    std::cout << "Rank "<<rank<<" PASSED\n";
    }
