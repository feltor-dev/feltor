...
namespace toefl{
    ...
    class Poisson
    {
      ...
      public: 
         ...
         */
        void operator()( Vector< double,2>& phi, const double laplace);
        /*! @brief Compute prefactors for ne, ni and nz in local poisson equation
         *
         * @param phi   phi[0] multiplies with ne, phi[1]  with ni, phi[2] with nz
         * @param laplace   The laplacian in fourier space
         */
        void operator()( Vector< double,3>& phi, const double laplace);
        /*! @brief Compute Gamma_i
         *
         * @param laplace The laplacian in fourier space
         * @return Gamma_i
         */
        inline double gamma_i( const double laplace);
        /*! @brief Compute Gamma_z
         *
         * @param laplace The laplacian in fourier space
         * @return Gamma_z
         */
        inline double gamma_z( const double laplace);
    ...
    void Equations::operator()(QuadMat< complex, 2>& c, const complex dx, const complex dy) 
    {
        Vector< double,2> phi;
        L = (dx*dx + dy*dy).real(); 
        p( phi, L); //prefactors in Poisson equations (phi = phi[0]*ne + phi[1]*ni)
        curv = (kappa_x*dx + kappa_y*dy); //note that curv is complex
        complex P = -g_e*dy + curv + dd;
        complex Q = -g_i*dy*p.gamma_i(L) + curv( p.gamma_i(L) + 0.5 *p.gamma2_i(L));

        c(0,0) = P*phi[0] - curv - dd + nu*L*L; c(0,1) = P*phi[1];
        c(1,0) = Q*phi[0];                      c(1,1) = Q*phi[1] + tau_i*curv + nu*L*L;
    }
    void Equations::operator()(QuadMat< complex, 3>& c, const complex dx, const complex dy) 
    {
        ...
        complex P = -g_e*dy + curv + dd;
        complex Q = -g_i*dy*p.gamma_i(L) + curv( p.gamma_i(L) + 0.5 *p.gamma2_i(L));
        complex R = -g_z*dy*p.gamma_z(L) + curv( p.gamma_z(L) + 0.5 *p.gamma2_z(L));

        c(0,0) = P*phi[0] - curv - dd + nu*L*L; c(0,1) = P*phi[1];                       c(0,2) = P*phi[2];
        c(1,0) = Q*phi[0];                      c(1,1) = Q*phi[1] + tau_i*curv + nu*L*L; c(1,2) = Q*phi[2];
        c(2,0) = R*phi[0];                      c(2,1) = R*phi[1];                       c(2,2) = R*phi[2] + tau_z*curv + nu*L*L;
    }

    void Poisson::operator()( Vector< double, 2>& c, const double laplace)
    {
        double rho = - a_i*mu_i*laplace/(1.+ tau_i*mu_i*laplace);
        c[0] = 1./rho;
        c[1] = -a_i*gamma_i(laplace)/rho;
    }
    void Poisson::operator()( Vector< double, 3>& c, const double laplace)
    {
        double rho = (a_i*mu_i/(1.- tau_i*mu_i*laplace) + a_z*mu_z/(1.-tau_z*mu_z*laplace))*laplace;
        c[0] = 1./rho;
        c[1] = -a_i*gamma_i(laplace)/rho;
        c[2] = -a_z*gamma_z(laplace)/rho;
    }
    double Poisson::gamma_i( const double laplace)
    {
        return (1./(1. - 0.5*tau_i*mu_i*laplace));
    }
    double Poisson::gamma_z( const double laplace)
    {
        return (1./(1. - 0.5*tau_z*mu_z*laplace));
    }
    double Poisson::gamma2_i( const double laplace)
    {
        double gamma = gamma_i(laplace);
        return 0.5*tau_i*mu_i*laplace*gamma*gamma;
    }
    double Poisson::gamma2_z( const double laplace)
    {
        double gamma = gamma_z(laplace);
