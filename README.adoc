...
First, we need to install Niels Lohmann's popular json library (header-only, distributed under the MIT License),
which on linux is available as `nlohmann-json3-dev` through the package managment system.
For a manual build check the instructions on https://www.github.com/nlohmann/json[JsonCpp].
[source,sh]
----
git clone https://www.github.com/nlohmann/json
cd ~/include
ln -s path/to/json/include
----
...
# shared memory version using glfw and netcdf
toefl: toefl.cpp toefl.h parameters.h diag.h
    $(CC) $(OPT) $(CFLAGS) $< -o $@ $(INCLUDE) $(GLFLAGS) $(LIBS) -DWITH_GLFW -g

# shared memory version using netcdf without glfw
toefl_hpc: toefl.cpp toefl.h parameters.h diag.h
    $(CC) $(OPT) $(CFLAGS) $< -o $@ $(INCLUDE) $(LIBS) -g

# mpi version using netcdf without glfw
...
Our simulation codes typically depend on json for parameter input,
...
* In Github desktop: `File -> Clone repository...` clone `https://github.com/nlohmann/json`
* In `VC++ Directories -> Include Directories` click on `Edit` Then add the line
`path\to\json\include`
