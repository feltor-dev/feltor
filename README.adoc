...
We suggest to create a Property Sheet in https://visualstudio.microsoft.com/[Visual Studio]. The Property Sheet can then be conveniently added to any project that includes the FELTOR library
...
That's it. Now, in order to test our Feltor Property Sheet let us add a source file to the project and compile
...
In order to add a project to an existing solution select `File -> Add->New Project...`. 
You can then add a cpp file: Right click on `Source Files` and add the file as an existing item.
Add the Feltor Property Sheet by switching to the `Property Manager` again and clicking `Add Existing Property Sheet`.
...
You will need to download these additional libraries and adapt the project properties accordingly. 
...
* Execute the file `path\to\jsconcpp\amalgamate.py`, which will generate a `dist` folder containing `jsconcpp.cpp` and two header files.
* Open the Property Sheet and add `path\to\jsoncpp\dist` to `VC++ Directories-> Include Directories`
* In the Solution Explorer Right click on `Source Files -> Add -> Existing Item -> path\to\jsconcpp\dist\jsoncpp.cpp`

Glfw3::

* Clone `https://github.com/feltor-dev/draw`
* Download and extract the Windows binaries from https://www.glfw.org/download.html
* Add `path\to\glfw-3\include` and `path\to\folder_containing_draw` to `VC++ Directories-> Include Directories`
...
* Add `path\to\netCDF\include` to `VC++ Directories-> Include Directories`
