...
----
____
Alternatively to the jsoncpp library, one can also use Niels Lohmann's popular
json parser, which is header-only (and also distributed under an MIT License).
On linux it is available through the `nlohmann-json3-dev` package or one can simply
download
[source,sh]
----
...
----
Then, one needs to overwrite `JSONLIB=-DDG_USE_JSONHPP` in the Makefile
____
