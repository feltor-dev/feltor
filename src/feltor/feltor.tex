...
\begin{align}
[f,g]_\perp \equiv [f,g]_{RZ} &= \pm \left(\partial_R f\partial_Z g - \partial_Z f\partial_R g\right) \\
...
\begin{align}
\KK \approx \KB
  &\approx  -\frac{1}{B^2}\frac{\partial B}{\partial Z}\ehat_R +\frac{1}{B^2} \frac{\partial B}{\partial R}\ehat_Z &
...
\subsubsection{Flutemode ordering} \label{sec:flutemode}
When applying flutemode ordering to the gyro-kinetic one-form directly it turns
out that the angular momentum can be conserved exactly. This is what we do in the thermal code.
It consists of
\begin{align}
    B \approx B_{\hat \varphi}:=& \frac{R_0 I}{R}
    \\
    \KB :=& \frac{\ehat_\varphi \times \vn \ln B_{\hat \varphi}}{B_{\hat \varphi}}= - \frac{1}{B_{\hat \varphi}}\frac{ \partial_Z I}{I} e_R + \left(\frac{1}{B_{\hat \varphi}}\frac{\partial_R I}{I} - \frac{1}{R_0 I}\right) e_Z
    \\
    \KK :=& \frac{\vn \times \ehat_\varphi}{B_{\hat \varphi}} = \frac{-\hat e_Z/R}{B_{\hat \varphi}} = - \frac{1}{R_0 I} \hat e_Z
    \\
    \nc \KK :=& \frac{\partial_Z I}{R_0 I^2}
    \\
    \nc \frac{\vec B}{B_{\hat \varphi}} =& B_R\left(  \frac{1}{R_0 I} - \frac{1}{B_{\hat \varphi}}\frac{\partial_R I}{I}\right) = \frac{\psi_Z}{I} \left( \frac{1}{R} -  \frac{\partial_R I}{I} \right)
\end{align}
Note that if $I=I_0$ (typically the case) then $\KK = \KB$ and we automatically have the low-beta approximation.
\begin{align}
    \ehat_\varphi \times \vec v = -v_Ze_R +v_Re_Z
\end{align}
Contrary to the toroidal field approximation we do not need to use $\pm \ehat_\varphi$ here. The sign of $\vec B$ is taken care of automatically in $B_{\hat \varphi}$.
This means that the sign of the sheath coordinate is taken wrt to the geometric angle instead of $\bhat$.

In this ordering the parallel Ampere law is derived from the variation of the actual $B_\perp^2$ term.
Thus the parallel Ampere law is changed to
\begin{align}
    \left(\frac{\beta}{\mu_i}R^{-2}N_i - \frac{\beta}{\mu_e} R^{-2}n_e - \vn R^{-2} \np \right) (A_\parallel R)= \beta R^{-1}(N_i W_{\parallel,i} - n_e w_{\parallel,e})
\end{align}
We solve this equation for $(A_\parallel R)$ and then obtain $A_\parallel = (A_\parallel R)/R$ from it.
The magnetic energy likewise reads
\begin{align}
\vec B_\perp^2 = (\vn \times A_\parallel \ehat_\varphi)^2 = \left( \frac{\np (R A_\parallel)}{R}\right)^2 = (\partial_Z A_\parallel)^2 + ( \partial_R A_\parallel + R^{-1} A_\parallel)^2
\end{align}
...
"boundary":
{
    "wall" :
    {
        ...
        // or
        "nwall" : "floating" // $N_w$ is chosen as average $N$ right before wall region
...
For the "floating" wall boundary condition we choose $N_w$ as
\begin{align}
    N_{w,s} = \frac{1}{\int \d V \chi_w(1-\chi_w)}\int \d V \chi_w ( 1-\chi_w) N_s \equiv N_{avg,s}
\end{align}
Note that
\begin{align}
\int \d V N ( 1- \chi_w) = \int \d V F_N (1-\chi_w)^2 - \omega_w \int \d V \chi_w (1-\chi_w) ( N - N_w)
\nonumber\\
\approx& \int \d \vec A\cdot j - \omega_w V_{avg} (N_{avg} - N_w)
\end{align}
For the floating wall boundary the last term vanishes to exactly $0$. (Since the polarisation equations is
penalized it may not matter that $n_{w,e}\neq N_{w,i}$ is possible. \fixme{Confirm})
...
\end{minted}
\noindent
We also allow a polynomial fit for a flux-aligned density profile as
\begin{align} \label{eq:density_profile_polynomial}
    \bar \psi_p :=& 1-\frac{\psi_p}{\psi_{p,O}} \\
    n_{\text{prof}}(R,Z)=& \begin{cases}
        \sum_{n=0}^{N-1} c_n \bar \psi_p^n &\text{ for } \bar \psi_p < 1\\
        n_{\text{bg}} + q_0\exp\left( q_1 (\bar \psi_p - 1) + q_2 (\bar \psi_p - 1)^2 \right)&\text{ else }
    \end{cases}
\end{align}
This profile is a polynomial in the normalised flux $\bar \psi_p$ in the closed fieldline region and exponentially
decreases in the SOL to a constant background value.
The coefficients $q_0$, $q_1$ and $q_2$ are chosen such that the function is twice continuously differentiable at $\bar \psi_p =1$.
Typically this profile is used to fit an experimental profile.
\begin{minted}[texcomments]{js}
"density":
{
    "profile" :
    {
        ...
        "c" : [1, 2, 3, ...] // Coefficients in Eq.\eqref{eq:density_profile_polynomial}, the number of elements is infered
        "background" : 0.2 // $n_{bg}$ in Eq.~\eqref{eq:density_profile_polynomial},
        //also value to which to dampen the profile in Eq.\eqref{eq:initial_ne}
        //(see also nbc, minne and nwall)
    }
}
...
reads one or more previously generated simulation file(s) \mintinline{bash}{input0.nc ... inputN.nc} described in Section~\ref{sec:output_file} and writes into a
