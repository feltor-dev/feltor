...
\texttt{cd path/to/feltor/src/feltor}
\subsection{Compilation}
Compile \texttt{feltor.cu} (only shared memory, glfw or netcdf output)\\
\texttt{make feltor device=\{gpu,omp\}} \\
\\
Compile \texttt{feltor.cu} for shared memory system. Only {\it serial netcdf} output (no glfw). \\
\texttt{make feltor\_hpc device=\{gpu,omp\}}\\
\\
Compile \texttt{feltor.cu} for distributed memory systems. Only {\it serial netcdf} output (no glfw).\\
\texttt{make feltor\_mpi device=\{gpu,omp,skl\}} \\
\begin{tcolorbox}[title=Note]
    \texttt{feltor}, \texttt{feltor\_hpc} and \texttt{feltor\_mpi} are
    compilations of the same program for different hardware.
\end{tcolorbox}
Compile \texttt{feltordiag.cu} for shared memory systems. Needs {\it serial netcdf}\\
\texttt{make feltordiag device=\{gpu,omp,skl\}}
\subsection{Usage}
\texttt{./feltor input.json [output.nc] [initial.nc]} \\
\texttt{./feltor\_hpc input.json output.nc [initial.nc]} \\
\texttt{echo npx npy npz | mpirun -n np ./feltor\_mpi input.json output.nc [initial.nc]} \\
\texttt{./feltordiag output.nc diag\_output.nc}

\begin{tcolorbox}[title=Note]
\texttt{feltor\_mpi} expects the
...
\href{http://cfconventions.org/Data/cf-conventions/cf-conventions-1.7/cf-conventions.html}{CF Conventions CF-1.7}
 ...
 The programs \texttt{feltor}, \texttt{feltor\_hpc} and \texttt{feltor\_mpi}
 ...
 The programs \texttt{feltor\_hpc} and \texttt{feltor\_mpi} write results into
...
bath on top of it.
\begin{tcolorbox}[title=Note]
See the output \texttt{sne\_tt\_ifs} in \texttt{feltordiag} for how much mass
the source with the parameters below generates and compare to
\texttt{jsne\_tt\_fsa} to see how much mass is lost.
\end{tcolorbox}
\begin{minted}[texcomments]{js}
"source":
{
...
"FCI",
{
    ...
    "bc" : "along_field", // use ds\_bc\_along\_field function for parallel derivatives
    "bc" : "perp", // use perpendicular interpolation technique
    "periodify" : true
    ...
    // consistent but works well for small toroidal resolution
    // Does not have any effect for the "along\_field" mode
}
\end{minted}
...
\end{minted}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Steering the file output}
...
"output":
{
    ...
    "itstp"  : 4, // { \tt inner\_loop*itstp} is the number of
    ...
    // {\tt maxout$\cdot$itstp$\cdot$inner\_loop})
...
\subsection{Toroidal ExB angular momentum equation} \label{sec:vorticity_eq}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Parallel momentum balance}
...
\subsection{Parallel electron force balance}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Restarting} \label{sec:restart_file}
...
\begin{enumerate}
    ...
    \item providing an additional netcdf file \texttt{initial.nc}
        to \texttt{feltor}, \texttt{feltor\_hpc} or \texttt{feltor\_mpi} on the
        command line. This mode takes precedence over the firstl because it
        ignores the "init" field of the input file.
\end{enumerate}
...
\begin{tcolorbox}[title=Note]
    The restart file is only used to initialize $n_e$, $N_i$, $u_{\parallel,e}$ and
    $U_{\parallel,i}$ with the latest timestep, all parameters (magnetic field,
    profiles, resolutions, sources, etc.) except "init" are still read from the input
    json file.
The user is responsible that these are consistent with the paramters
in the existing \texttt{initial.nc} file.
\end{tcolorbox}
