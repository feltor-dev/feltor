...
to contain the field "rho_s" where $\rho_s$ in meters is given which will be
used to convert $R_0$ from meter to $\rho_s$.

If the "input" parameter is set to "params" then $R_0$ is assumed to already be
in units of $\rho_s$. The field "rho_s" does not need to be present in the "physical"
field then.
\end{tcolorbox}
...
"probes":
{
    "R": [90, 95, 100], // R coordinate in rho_s
    "Z": [0, 0, 0], // Z coordinate in rho_s
