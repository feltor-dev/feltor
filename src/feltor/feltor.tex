...
A simple choice is a purely toroidal magnetic field of the form %($I(\psi)=\psi=1$) 
\begin{align}
...
\label{}
\end{align}
\subsection{Parallel Resistivity, diffusion, profiles}\label{sec:dissres}
...
We start with initializing a flux-aligned profile
\begin{align}
    n_e(R,Z,\varphi, 0)= n_{prof}(R,Z) + \tilde n(R,Z,\varphi)
\end{align}
consisting of a symmetric background profile $n_{prof}(R,Z)$ and a perturbation
$\tilde n(R,Z,\varphi)$.
\subsubsection{Background profile}
Let us define an approximate Heaviside function
\begin{align}
  \Theta(x) := \frac{1}{2}\left( 1 + \tanh\left( \frac{x}{ \alpha} \right) \right)
  \label{eq:source_profile}
\end{align}
with a (small) width parameter $\alpha$
and a flux-aligned density profile as
\begin{align}
  n_{prof}(R,Z)=
      \left(n_0 + \triangle n_{peak}\frac{\psi_p(R,Z)} {\psi_p(R_0, 0)}\right) \Theta( -\psi_p) \Theta(Z-Z_X)
  \label{eq:density_profile}
\end{align}
If the magnetic field is toroidal the 
The second Heaviside is multiplied only if the equilibrium has an X-point.


\subsubsection{Blob and Straight blob}
We initialize a blob in the R-Z plane
\begin{align}
  n_{blob}(R,Z,0) = \triangle n \exp\left( -\frac{(R - R_0 - p_x a)^2 + (Z-p_ya)^2}{\sigma^2} \right)
  \label{}
\end{align}
Then, we use fieldline integration modulated by 
\begin{align}
  m_{blob}(\varphi) = \exp\left( -\frac{\varphi^2 }{\pi^2\sigma_z^2} \right)
  \label{}
\end{align}
...
\begin{align}
    n_{zonal}(R,Z,\varphi) = A |\sin (2\pi k_\psi \psi_p)|
\end{align}

\subsection{Particles sources}
The idea for the source terms $S$ is to keep the profile in the core of our domain
where our model does not apply.
We then define the particle source for electrons as
\begin{align}
  S_{n_e}(R,Z,\varphi, t) = \omega_s
    (n_{prof}(R,Z) - n_e(R,Z,\varphi, t))\Theta( - \psi_p(R,Z) + \psi_{p,\min} ),
...
source  & float & 0     & - & profile source rate $\omega_s$ in Eq.~\eqref{eq:electron_source} \\
    boxscaleRp  & float & 1.1     & 1.05 & $\varepsilon^{R+}$ scale right boundary in units of $a$\\
    boxscaleRm  & float & 1.1     & 1.05 & $\varepsilon^{R-}$ scale left boundary in units of $a$ \\
    boxscaleZp  & float & 1.1     & 1.05 & $\varepsilon^{Z+}$ scale upper boundary in units of $ae$ \\
    boxscaleZm  & float & 1.2     & 1.05 & $\varepsilon^{Z-}$ scale lower boundary in units of $ae$ \\
...
equilibrium & string & "solovev"  & "solovev" & type of magnetic field
equilibrium, one of "solovev", "toroidal", or "circular"
(all geometry parameters will be ignored, except major and minor radius and elongation)) \\
\bottomrule
\end{longtable}
...
\rowcolor{gray!50}\textbf{Name} &  \textbf{Type} & \textbf{Example} & \textbf{Default} & \textbf{Description}  \\ \midrule
    ...
    alpha     & float  & 0.02 & - & Width $\alpha$ of Heaviside profile in Eq.~\eqref{eq:source_profile} \\
...
aligned          & Dataset & 1 (energy\_time) & Alignment paramter $\int\dV \ln n_e\Delta_\parallel n_e =  -\int\dV (\nabla_\parallel n_e)^2 /n_e$\\
\bottomrule
\end{longtable}
