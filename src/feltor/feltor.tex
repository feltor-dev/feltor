...
to contain the field "rhos" where $\rho_s$ in meters is given which will be
used to convert $R_0$ from meter to $\rho_s$.

If the "input" parameter is set to "params" then $R_0$ is assumed to already be
in units of $\rho_s$. The field "rhos" does not need to be present in the "physical"
field then.
\end{tcolorbox}
...
"physical":
{
    ...
    "rhos" : 1e-3 // $\rho_s$ in units of meter
}
\end{minted}
...
introduced in $\rho_s$ units, while the toroidal angle is given in radian. The
...
relevant unless the size exceeds $10^4$.
\begin{minted}[texcomments]{js}
"probes":
{
    "R": [90, 95, 100], // R coordinate in rhos
    "Z": [0, 0, 0], // Z coordinate in rhos
    "P": [0, 0, 3] // phi coordinate in radian
}
\end{minted}
\begin{tcolorbox}[title=Note]
