...
\begin{table*}[htbp]
\caption{Definitions of geometric operators with $b^i$ the contra-variant components of $\bhat$ and $g^{ij}$ the contra-variant elements of the metric tensor.
...
\subsection{Output equations flags}\label{sec:output_flags}
In the input file, inside of the "output" input parameters, a new variable defined "equations" allows to turn on and off the different variables that correspond to the different equations that can be studied with FELTOR at the moment. These equations are described in section \ref{sec:output}, and the definitions are in the equations titles.
\begin{minted}[texcomments]{js}
"output":
{"equations" :
{ "Basic": true,
...
\subsection{Mass conservation, "Mass-conserv": true}
...
\subsection{Energy theorem, "Energy-theorem": true}
...
\subsection{Toroidal ExB angular momentum equation, "Toroidal-momentum": true} \label{sec:vorticity_eq}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Conservation of Currents (COCE), "COCE": true}\label{sec:COCE}
...
\bottomrule
\end{longtable}
Some of these definitions have "\_gf" in their definitions, as they are defined with the gyro-fluid ion density. This quantities are also saved without this label, when using the electron fluid density $n_e$ instead of $N_i$.


\subsection{Parallel momentum balance, "Parallel-momentum": true}
...
\subsection{Parallel electron force balance, "Parallel-momentum": true}
...
\subsection{Zonal flow energy, "RS": true}
...
The usage requires a \mintinline{bash}{configuration.json} where parameters for the diagnostic can be introduced. Some of this inputs are "Neta", "Npsi" and "n" for the dimensions and DG method order for the transformation to the magnetic coordinate system. 
\begin{tcolorbox}[title=Note]
...
Z\_2dX  & Dataset & 3 (time, psi, theta) & Convoluted toroidal average Eq.~\eqref{eq:cta} in the magnetic plane \\
\bottomrule
\label{table:diag}
\end{longtable}
where Z $\in$ \{X, Y\_tt\}
...
There are also "DIAG\_OUTPUTS" flags in \mintinline{bash}{configuration.json} to specify which kind of diagnosed quantities are wanted:
\begin{itemize}
\item "1dStatics": If true, save variables that only depend on psi in table \ref{table:diag}.
\item "fsa1d": If true, save \_fsa in table \ref{table:diag}.
\item "fsa2d":  If true, save \_fsa2d in table \ref{table:diag}.
\item "cta2d":  If true, save \_cta in table \ref{table:diag}.
\item "cta2dX":  If true, save \_2dX in table \ref{table:diag}.
\item "fluc2d":  If true, save \_fluc2d in table \ref{table:diag}.
\item "ifs":  If true, save \_ifs in table \ref{table:diag}.
\item "std\_fsa":  If true, save \_std_fsa in table \ref{table:diag}.
\item "ifs\_lcfs":  If true, save \_ifs_lcfs in table \ref{table:diag}.
\item "ifs\_norm":  If true, save \_ifs_norm in table \ref{table:diag}.
\end{itemize}
