...
(the equilibrium equation in toroidally symmetric, ideal MHD,
...
\end{align}
Furthermore, we have
\begin{align} \label{eq:curl_B}
  \nabla_\parallel f(R,Z) = \frac{R_0}{RB}[f,\psi_p]_{RZ}\Rightarrow \nabla_\parallel \ln B = \frac{R_0}{RB^2}\left[B, \psi_p\right]_{RZ} = -\vec\nabla\cdot\bhat,\\
    \vec\nabla\times \vec B = \frac{R_0}{R}\left[ I_Z \ehat_R - I_R\ehat_Z + Rj_\varphi\ehat_\varphi  \right]\equiv \vec j.
\end{align}
Note that the poloidal equilibrium current is given by $R_0 \vec \nabla I\times \ehat_\varphi/R$
and the toroidal current by $R_0 \Delta^* \psi_p$.
Eq.~\eqref{eq:curl_B} together with Eq.~\eqref{eq:curl_curvature}
...
This program reads in the output file above and computes
...
\rangle_\varphi$ (Eq.~\eqref{eq:phi_average} where X is \{Ne, Ni, Ue, Ui, phi,
vor\} corresponding to $\{ n_e, N_i, u_e, U_i, \phi, \Delta\phi\}$ \\
dX\_mp           & Dataset & 3 (time,y,x) & Fluctuation level on midplane ($\varphi\equiv \pi$) $\delta X := X(R,Z,\pi) - \langle X\rangle_{\psi_{p}}$ where X is \{Ne, Ni, Ue, Ui, phi,
vor\} corresponding to $\{ n_e, N_i, u_e, U_i, \phi, \Delta\phi\}$ \\
\bottomrule
\end{longtable}
