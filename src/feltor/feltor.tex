...
\begin{align}
 -\vec{\nabla} \cdot\left(\frac{N_i}{\Omega_i B} \vec{\nabla}_\perp \phi\right) &=  \Gamma_{1,i} N_i -n_e, \quad \Gamma_{1,i}^{-1} := 1-\frac{m_i T_i}{2q_i^2 B_0^2} \Delta_\perp , \\
...
\begin{align}
  \psi_e := \phi,\quad \psi_i&:= \Gamma_{1,i} \phi - \frac{m_i }{2 q}\frac{(\vec \nabla_\perp\phi)^2}{B^2}
\end{align}
...
Note that we let the perpendicular diffusion act on $W$ instead of $U$ for
numerical reasons.
...
\begin{align*}
\psi_p(R,Z) &:=0.5((R-R_0)^2 + Z^2)\\
I_p(R,Z)&:=I_0
\end{align*}
with $R_0=10$ and $I_0=20$ and a simulation box $[R_0-a,R_0+a]\times[-a,a]\times[0,2\pi]$.
We then symbolically compute source terms that we insert to the right hand side of
the corresponding equation in code and simulate from $t=0...10^{-3}$.
