...
The purpose of this document is to describe the (usage of) programs
contained in \mintinline{bash}{path/to/feltor/src/feltor} such
that there is no need to look
into the actual source code and to connect
...
\end{align}
\end{subequations}
with $N_w = 1$ and $U_{\parallel,w}=0$.
...
\begin{align}
    ...
    \partial_t W_{\parallel,i} &= F_{U_i} +\omega_s \chi_s \left(\pm\sqrt{1+\tau} - U_{\parallel,i} \right) \\
    \partial_t w_{\parallel,e} &= F_{u_e} +\omega_s \chi_s \left(\pm\sqrt{1+\tau}\exp(-\phi) - u_{\parallel,e} \right)
    \text{ "bohm" } \label{eq:bohm_sheath}\\
    \partial_t w_{\parallel,e} &= F_{u_e} +\omega_s \chi_s \left(\pm\sqrt{1+\tau} - u_{\parallel,e} \right)\text{ "insulating" } \label{eq:insulating_sheath}
\end{align}
\end{subequations}
where $\omega_s$ is the sheath penalization parameter, the sign is such that the velocity is always flowing out of the plasma and $N_{sh}$ is obtained by extrapolating $N$ along the magnetic field line with
...
"boundary" :
{
    "sheath" :
    {
        ...
        "boundary" : 0.2, // width of the sheath $\eps_s$ away from the wall
        // in units of $a$ in Eq.\eqref{eq:sheath}
        "alpha" : 0.25, // Transition width $\alpha$
        // in units of $a$ in Eq.\eqref{eq:sheath}.
...
"init":
{
    "type" : "restart", // only the file parameter is needed (see Section \ref{sec:restart})
...
"init":
{
    ...
    "potential" : "zero_pol",// $N_i = n_e$ (polarization charge is 0)
    "potential" : "zero",    // $N_i = \left( 1 - \frac{1}{2}\tau_i\mu_i\Delta_\perp \right) n_e= \Gamma_{1,i}^{-1} n_e $ (ignored if $\tau_i = 0$)
    "potential" : "balance", // $N_i = \left(1+\frac{1}{2}\tau_i\mu_i\Delta_\perp\right)n_e\approx \Gamma_{1,i}n_e$ (ignored if $\tau_i = 0$)
    // \ldots ntilde, profile and damping parameters
}
\end{minted}
\begin{minted}[texcomments]{js}
"init":
{
    "type" : "ni",  // $N_i = (n_{prof}(R,Z) + \tilde n(R,Z,\varphi)) D(R,Z)$
    "potential" : "zero_pol",// $n_e = N_i$ (polarization charge is 0)
    "potential" : "zero",    // $n_e = \Gamma_{1,i}N_i$ (ignored if $\tau_i = 0$)
    "potential" : "balance", // $n_e = \left(1-\frac{1}{2}\tau_i\mu_i\Delta_\perp\right)N_i = \Gamma_{1,i}^{-1}N_i$ (ignored if $\tau_i = 0$)
    // \ldots ntilde, profile and damping parameters
}
\end{minted}
\noindent
...
\begin{tcolorbox}[title=Note]
    ...
    is a clear warning that \mintinline{json}{nu_perp} is too large for the chosen resolution.
\end{tcolorbox}
...
where $t_1 - t_0 = $ \mintinline{c}{\tt dt*inner_loop*itstp} and {\tt itstp} is the number of discretization points\\
...
\noindent Usage
\mint{bash}{./feltordiag input0.nc ... inputN.nc output.nc}
...
the corresponding equation in code (\mintinline{c}{manufactured.h}) and simulate from $t=0...10^{-3}$.
