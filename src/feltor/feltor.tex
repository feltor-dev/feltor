...
\begin{align}
  n_{prof}(R,Z)=
      n_0 + \triangle n_{peak}\frac{\psi_p(R,Z) \Theta( -\psi_p) \Theta(Z-Z_X)} {\psi_p(R_0, 0)}
...
\begin{align}
    \tilde n_{zonal}(R,Z,\varphi) = A |\sin (2\pi k_\psi \psi_p)|
\end{align}
...
Both will write results into an output file.
...
mu         & float & -0.00272121& - & $\mu_e :=-m_e/m_i$. One of $\left\{ -0.000544617, -0.000272121, -0.000181372 \right\}$\\
...
perturbation $\tilde n$ in \eqref{eq:initial_ne} "zonal",
