...
\begin{align}
  H(x) \approx \Theta(x) := \frac{1}{2}\left( 1 + \tanh\left( \frac{x-3\alpha}{ \alpha} \right) \right)
...
\begin{align} \label{eq:density_profile}
  n_{prof}(R,Z)=
      n_0 + \triangle n_{peak}\frac{\psi_p(R,Z) \Theta( -\psi_p(R,Z)) H(Z-Z_X)} {\psi_p(R_0, 0)}
\end{align}
...
\tilde n_e(R,Z,\varphi) &= \tilde n_{\text{zonal}}\Theta(-\psi_p)H(Z-Z_X)
\end{align}
...
rho\_max & float  & -6   & - & Source region boundary $\rho_{\max}$ in Eq.~\eqref{eq:electron_source} \\
\bottomrule
\end{longtable}
...
x                & Dataset & 1 & R-coordinate (computational space, size: $nN_x$)\\
y                & Dataset & 1 & Z-coordinate (computational space, size: $nN_y$)\\
