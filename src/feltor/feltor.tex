...
The dynamic viscosity $\mu\nu_\parallel$ is larger for ions than for electrons.
 ...
 determines the parallel resistive terms to $R_\parallel:= n_e\eta J_{\parallel,s}$ (This form both conserves parallel momentum and a quadratic energy dissipation term).
...
\begin{align}
    ...
    u_{\parallel,e, sh} &= s\sqrt{1+\tau}\exp(-\phi) \text{ "bohm" } \label{eq:bohm_sheath}\\
...
"init":
{
    "density" :
    {
        ...
        // same as choosing "const" profile with zero perturbation and no damping
...
\begin{align} \label{eq:density_profile}
    n_{\text{prof}}(R,Z)= n_{peak}\frac{\psi_p(R,Z) }{\psi_{p,O}} + n_{sep} \frac{\psi_{p,O} - \psi_p(R,Z)}{\psi_{p,O}}
\end{align}
...
\begin{align} \label{eq:profile_blob}
    n_{prof}(R,Z) = B + A \exp\left( -\frac{(R - R_0 - p_x a)^2 + (Z-p_ya)^2}{\sigma^2} \right)
\end{align}
\begin{minted}[texcomments]{js}
"density":
{
    ...
    {
        "type" : "gaussian",
        "background" : 1.0, // $B$ in Eq.\eqref{eq:profile_blob}
        //(see also nbc, minne and nwall)
...
"density":
{
    "damping" :
    {
        ...
        "background" : 0.1 // $n_{bg}$ value to which to dampen the profile in Eq.\eqref{eq:initial_ne}
        //(see also nbc, minne and nwall)
    }
}
\end{minted}
...
For the parallel advection terms $U_\parallel \npar N$ and $U_\parallel \npar
U_\parallel$ we can select several discretizations
\begin{minted}[texcomments]{js}
"advection":
{
    "parallel" : "centered" // centered differences for $U_\parallel \npar N$,
    // and $U_\parallel \npar U_\parallel$
    //  mandates artificial parallel diffusion to be stable (centered in space
    // is unconditionally unstable without diffusion).
    "parallel" : "upwind"  // 1st order upwind scheme for $U_\parallel \npar N$,
    // centered for $U_\parallel \npar U_\parallel$
    "parallel" : "upwind2"  // 2nd order upwind scheme for $U_\parallel \npar N$,
    // centered for $U_\parallel \npar U_\parallel$
}
\end{minted}
...
for the perpendicular terms and parallel viscosity of order 1 for the
density equations (the velocity equations already have a high physical
...
"regularization",
{
    ...
    // looks "out of focus"
    "nu_parallel_n" : 1000 // $\nu_{N,\parallel}$, the density artificial parallel
    // diffusion to stabilize the centered advection
    // (should be chosen in units of $R_0^2$, for example $\nu_{N,\parallel} = 10^{-2} R_0^2$)
}
\end{minted}
...
We also use the grid refinement technique (support operator)
in particular also when field-aligning the initial condition.
\begin{minted}[texcomments]{js}
"FCI",
{
    "refine" : [2,2],
