...
\begin{align}
  ...
  \label{}
\end{align}
which in turn initializes $A_\parallel = w_e=W_i = 0$
and initialize the electron density with
\begin{align} \label{eq:initial_ne}
    n_e(R,Z,\varphi, 0)= n_{prof}(R,Z) + \tilde n(R,Z,\varphi)
\end{align}
consisting of a toroidally symmetric background profile $n_{prof}(R,Z)$ and a perturbation
$\tilde n(R,Z,\varphi)$.
...
refineDS     & integer[2] & [10,10] & [10,10] & refinement factor in DS in X- and Y-direction\\
...
initphi   & string & "zero"  & "zero" & initial condition for $\phi$ and thus $N_i$ (Eq.~\eqref{eq:initphi}: "zero" = 0
...
rho\_psip & float  & -6   & - & Source region in Eq.~\eqref{eq:source_profile} \\
\bottomrule
\end{longtable}
