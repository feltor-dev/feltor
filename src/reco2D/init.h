...
std::map<std::string, std::function< std::array<std::array<DVec,2>,2>(
    Asela<Geometry, IDMatrix, DMatrix, DVec>& f,
    const Geometry& grid, const asela::Parameters& p)
> > initial_conditions =
{
    { "zero",
        []( Asela<dg::x::CartesianGrid2d, dg::x::IDMatrix, dg::x::DMatrix, dg::x::DVec>& f,
            const dg::x::CartesianGrid2d& grid, const aslea::Parameters& p)
        {
    ...
    { "harris",
            ...
            const dg::x::CartesianGrid2d& grid, const aslea::Parameters& p)
        {
            ...
            y0[0][0] = y0[0][1] = y0[1][0] = y0[1][1] =
                dg::construct<DVec>(dg::evaluate( dg::zero, grid));
            ...
            dg::blas2::gemv( -1./p.beta, asela.laplacianM(), apar, 0., y0[1][0]);
    ...
    { "island",
            ...
            const dg::x::CartesianGrid2d& grid, const aslea::Parameters& p)
        {
            ...
            double kxp = M_PI/p.lxhalf/2.;
            dg::x::DVec apar = dg::evaluate( [=](double x, double y){ return
                cos(kxp*x)*( 1./kx*ln( cosh(kx*x) + 0.2*cos( kx*y) ) - p.amp1*cos( ky*y));
                }, grid);
            dg::blas2::gemv( -1./p.beta, asela.laplacianM(), apar, 0., y0[1][0]);
            dg::blas1::axpby(1., y0[1][0], 1./p.mu[0], apar, y0[1][0]);
            // if we include Gamma A again we must take care here
