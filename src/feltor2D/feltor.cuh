...
template<class Grid>
Feltor<Matrix, container, P>::Feltor( const Grid& g, eule::Parameters p, solovev::GeomParameters gp, const Grid& gridfordz): 
    ...
    dzy( 4, chi),curvy(dzy), 
    dzDIR_(solovev::Field(gp), gridfordz, gp.rk4eps,solovev::PsiLimiter(gp), dg::DIR),
...
void Feltor<M, V, P>::energies( std::vector<V>& y)
{
    ...
    //         dzNEU_.set_boundaries( dg::NEU, 0, 0);
        if( p.nu_parallel !=0)
        {
            ...
            dzNU_.dzz2d(y[i+2],omega);                                          //dz^2 U 
...
void Feltor<Matrix, container, P>::operator()( std::vector<container>& y, std::vector<container>& yp)
{
    ...
    */
    ...
    for( unsigned i=0; i<2; i++)
    {
        ...
        //energies( y); //make outside in feltor.cu!
