...
struct Esol
{
    ...
    {
        std::vector<unsigned> number = m_multigrid.direct_solve( m_multi_g1, yp, y, m_p.eps_gamma1);
    ...
    void solveSne(double t, const container& SNi, const container& potential, const container& y0, container& Sne)
    {
        if (m_p.source_rel == "zero-pol" || m_p.source_rel == "finite-pol")
        {
            ...
            std::vector<unsigned> number = m_multigrid.direct_solve( m_multi_g1, m_omega, SNi, m_p.eps_gamma1);
...
Esol< Geometry, M,  container>::Esol( const Geometry& grid, const Parameters& p ):
    ...
    m_prof(dg::evaluate( dg::TanhProfX(p.lx*p.xfac_sep, p.sigma_p,-1.0, p.bgprofamp, p.profamp), grid)), 
...
void Esol<G,  M,  container>::operator()( double t, const std::array<container,2>& y, std::array<container,2>& yp)
{
    ...
    dg::blas2::symv( m_lapMperp, m_iota, m_omega); //-nabla_perp^2 rh*(ne-bgprofamp-profamp)
