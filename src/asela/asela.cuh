...
struct Asela
{
     ...
     * @param gp the geometry parameters
     */
    Asela( const Geometry& g, eule::Parameters p, solovev::GeomParameters gp);
    /**
...
Asela<Grid, DS, Matrix, container>::Asela( const Grid& g, Parameters p, solovev::GeomParameters gp): 
...
double Asela<G, DS, M, V>::add_parallel_dynamics( std::vector<V>& y, std::vector<V>& yp)
{
  ...
  //compute chi = (tau_e(1+lnN_e)+phi + 0.5 mu U^2)
        ...
        //compute omega = NU
        dg::blas1::pointwiseDot( npe[i], u[i], omega); //N U   
        Dpar[i+2] = z[i]*p.mu[i]*dg::blas2::dot(omega, w3d, lambda);      //Z*N*U nu_para *(ds^2 U ) or Z*N*U nu_para *(ds^2 w )
    ...
    */
    ...
    for( unsigned i=0; i<2; i++)
    {
        ...
        {        
            ...
            dg::blas1::axpby( -p.tau[i], curvy[2+i], 1., yp[2+i]);     //dtw = dtU - tau K(U)
