...
void Feltor<Matrix, container, P>::operator()( std::vector<container>& y, std::vector<container>& yp)
{
    ...
    */
    ...
    //+D/C (phi+ tau_e*ne)(1+ne)
    dg::blas1::axpby(1.,phi[0],p.tau[0],logn[0],omega); //omega = phi - lnNe
    dg::blas1::pointwiseDot(omega,npe[0],lambda); //(phi - lnNe)*Ne
    dg::blas1::axpby(p.d/p.c,lambda,1.0,yp[0]); //D/C(phi - omega)Ne
//     dg::blas1::pointwiseDot(omega,y[0],lambda); //
//     dg::blas1::axpby(p.d/p.c,lambda,1.0,yp[0]); //D/C(phi - omega)Ne*logn
    t.toc();
#ifdef MPI_VERSION
    int rank;
