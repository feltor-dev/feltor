...
int main( int argc, char* argv[])
{
    ...
    ////////////////////////////////set up computations///////////////////////////
    dg::CartesianMPIGrid2d grid( 0, p.lx, 0, p.ly, p.n, p.Nx, p.Ny, p.bc_x, p.bc_y, comm); // dg::CartesianGrid2d grid( 0, p.lx, 0, p.ly, p.n, p.Nx, p.Ny, p.bc_x, p.bc_y);
    //create RHS 
    dg::Diffusion< dg::CartesianMPIGrid2d, dg::MDMatrix, dg::MDVec > diffusion( grid, p); // dg::Diffusion< dg::CartesianGrid2d, dg::DMatrix, dg::DVec > diffusion( grid, p);
    ...
    std::vector<dg::MDVec> y0(3, dg::evaluate(dg::one,grid) );
    ...
    size_t start[3] = {0, coords[1]*count[2], coords[0]*count[3]};
    size_t count[3] = {1, grid.n()*grid.Ny(), grid.n()*grid.Nx()};
    dg::MDVec transfer( dg::evaluate(dg::zero, grid));
    dg::DVec transferD( dg::evaluate(dg::zero, grid.local()));
    ...
    dg::blas1::transfer( transfer, transferH);
