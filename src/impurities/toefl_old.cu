...
int main( int argc, char* argv[])
{
    ...
    //create RHS
    if(rank==0) std::cout << "Constructing Diffiusion ...\n";
    dg::Diffusion< dg::CartesianMPIGrid2d, dg::MDMatrix, dg::MDVec > diffusion( grid, p);
    if(rank==0) std::cout << "Constructing TOEFLI ...\n";
    ...
    dg::CONSTANT prof( 0. );
    std::vector<dg::MDVec> y0(3, dg::evaluate( prof, grid));;
...
//  err = nc_create( argv[2],NC_NETCDF4|NC_CLOBBER, &ncid);
    ...
    dg::Grid2d<double> grid_out( 0, p.lx, 0, p.ly, p.n, p.Nx, p.Ny, p.bc_x, p.bc_y) ;
    ...
    for( unsigned i=0; i<p.maxout; i++)
    {
        ...
        err = nc_put_vara_double( ncid, dataIDs[3], start, count, transferH.data() );
        transferD = toeflI.potential()[0];
        dg::blas2::symv( diffusion.laplacianM(), transferD, y0[1]);
        dg::blas1::transfer( y0[1], transferH);
        err = nc_put_vara_double( ncid, dataIDs[4], start, count, transferH.data() );
        err = nc_put_vara_double( ncid, tvarID, start, count, &time);
